...
def generate_pac_content(port_number=None, fallback_direct=True):
    ...
    lines = [
        ...
        f"    // 1. Conexões para a rede local, intranet e IPs privados navegam direto (DIRECT)",
        ...
        f"        shExpMatch(host, '10.*') ||",
        f"        shExpMatch(host, '172.16.*') ||",
        f"        shExpMatch(host, '192.168.*')) {{",
        f"        return 'DIRECT';",
        f"    }}",
        f"",
        f"    // 2. Direciona todo o tráfego externo para o Proxy Squid!",
        f"    return '{proxy_rule}';",
        f"}}",
        f""
    ]
