...
def execute_remote_ssh(cfg):
    ...
    commands = [
        ...
        f"cd {cfg['remote_dir']} && ./venv/bin/python manage.py shell -c \"from squid.squid_sync import apply_squid_changes; print('Squid Sync:', apply_squid_changes())\"",
        "systemctl restart squidpanel",
        "systemctl reload squid || sudo squid -k reconfigure || true",
        "systemctl is-active squidpanel squid"
    ]

    full_cmd = " && ".join(commands[:3]) + " ; " + " ; ".join(commands[3:])
